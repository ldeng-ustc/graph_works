...
#include "common/utils/rocksdb/autovector.h"
#include "common/utils/rocksdb/port.h"
...
class BinaryHeap {
 public:
  BinaryHeap() {}
  explicit BinaryHeap(Compare cmp)
      : cmp_(std::move(cmp)) {}
 ...
 private:
  ...
  void downheap(size_t index) {
    ...
    while (1) {
      ...
      } else if (right_child < data_.size() && cmp_(data_[left_child], data_[right_child])) {
