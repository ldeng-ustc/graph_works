...
#include "common/utils/rocksdb/rocksdb_namespace.h"
...
class autovector {
 public:
  ...
  reference emplace_back(Args &&...args) {
    if (num_stack_items_ < kSize) {
      return *(new ((void *)(&values_[num_stack_items_++])) value_type(std::forward<Args>(args)...));
    } else {
  ...
  void emplace_back(Args &&...args) {
    if (num_stack_items_ < kSize) {
      new ((void *)(&values_[num_stack_items_++])) value_type(std::forward<Args>(args)...);
    } else {
 ...
 private:
  ...
  alignas(alignof(value_type)) char buf_[kSize * sizeof(value_type)];  // the first `kSize` items
  pointer values_;
...
autovector<T, kSize> &autovector<T, kSize>::assign(const autovector<T, kSize> &other) {
  values_ = reinterpret_cast<pointer>(buf_);
...
template <class T, size_t kSize>
autovector<T, kSize> &autovector<T, kSize>::operator=(autovector<T, kSize> &&other) {
  values_                = reinterpret_cast<pointer>(buf_);
