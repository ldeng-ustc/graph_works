...
class Futex {
 public:
  ...
  bool try_lock_for(const std::chrono::duration<Rep, Period> &timeout_duration) {
    const struct timespec timeout = {
        ...
        .tv_nsec = (timeout_duration % std::chrono::seconds(1)) / std::chrono::nanoseconds(1)};
    while (true) {
      __sync_fetch_and_add(&num_using, 1);
      if (__sync_bool_compare_and_swap(&futexp, 0, 1)) break;
      int ret = futex(&futexp, FUTEX_WAIT, 1, &timeout, nullptr, 0);
      __sync_fetch_and_sub(&num_using, 1);
      if (ret == -1 && errno != EAGAIN && errno != ETIMEDOUT) throw std::runtime_error("Futex wait error.");
 ...
 private:
  int               futexp;
  int               num_using;
  inline static int futex(int *uaddr, int futex_op, int val, const struct timespec *timeout, int *uaddr2, int val3) {
