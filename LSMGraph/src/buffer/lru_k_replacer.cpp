...
LRUKNode::LRUKNode(const size_t k)
    : history_(k + 1)
    , k_(k) {
  k_distance_ = INF;
}

bool LRUKNode::operator<(const LRUKNode &rhs) const {
  return false;
}
...
bool LRUKNode::Valid() const {
  return k_ != INF;
}

void LRUKNode::SetInvalid() {
  k_ = INF;
}
...
LRUKImpl::LRUKImpl(const size_t node_num, std::vector<LRUKNode> &node_ref)
    : node_ref_(node_ref)
    , r_heap_(node_num + 1)
    , frame_pos_map_(node_num, Pos{nullptr})
    , frame_pos_type_(node_num, 1) {
  r_heap_[0] = 0;
}
...
LRUKImpl::ListNode *LRUKImpl::ListEvict() {
...
void LRUKImpl::RemoveFromHeap(const size_t pos) {
  LSMG_ASSERT(pos != INF, "try to remove an invalid frame");
...
LRUKReplacer::LRUKReplacer(const size_t num_frames, const size_t k)
    : node_store_(num_frames, LRUKNode{k})
    , node_heap_(num_frames, node_store_)
...
void LRUKReplacer::RecordAccess(const frame_id_t frame_id, [[maybe_unused]] AccessType access_type) {
  LSMG_ASSERT(static_cast<size_t>(frame_id) < replacer_size_, "invalid frame id!");
...
void LRUKReplacer::Remove(const frame_id_t frame_id) {
  LSMG_ASSERT(static_cast<size_t>(frame_id) < replacer_size_, "invalid frame id!");
  std::lock_guard lg(latch_);
  LSMG_ASSERT(node_store_[frame_id].Evictable(), "try to evict a non-evictable frame");
