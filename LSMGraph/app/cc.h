#ifndef LSMG_CC_HEADER
#define LSMG_CC_HEADER
...
namespace lsmg {
inline int run_cc(LSMGraph &gStore, const SequenceNumber_t seq = MAX_SEQ_ID) {
  ...
  std::function<int32_t(VertexId_t)> F = [&](VertexId_t src) {
    ...
    for (; it_A.valid(); it_A.next()) {
      ...
      if (values[src] < values[dst]) {
        write_min(&values[dst], values[src]);
